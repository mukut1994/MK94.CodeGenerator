...
public static class Extensions
{
    ...
    public static HashSet<Type> GetImportedTypes(FileDefinition file)
    {
        ...
        var websocketTypes = from api in file.Types
                             from method in api.Methods
                             from socket in method.MethodInfo.GetCustomAttributes<RepliesWithWebsocketAttribute>()
                             select new[] { socket.ReceiveType, socket.ResponseType };

        return propDefs
            .Concat(apiRetTypes)
            .Concat(apiArgTypes)
            .Concat(websocketTypes.SelectMany(x => x))
